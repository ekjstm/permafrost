...
public class Struct {
    ...
    public final long address() {
        ...
        long start = ((sun.nio.ch.DirectBuffer) thisBuffer).address();
            ...
            if (struct == null) {
                try {
                    if (structClass == null) {
                        ...
                        if (structClass == null)
                            throw new RuntimeException("Struct class: "
    ...
    private int updateIndexes(int alignment, int nbrOfBits, int capacity) {
        ...
        if (_alignment < alignment) {
            _alignment = Math.min(alignment, MAXIMUM_ALIGNMENT);
        }
        return offset;
    }
     ...
     */
    public class Enum8<E extends Enum<?>> extends Member {
        private final int _mask;
        ...
        private final List<E> _enumValues;

        public Enum8(List<E> enumValues) {
            this(enumValues, 8);
        }

        public Enum8(List<E> enumValues, int nbrOfBits) {
            super(1, nbrOfBits, 8);
        ...
        public E get() {
            if (_mask == 0xFF) { // Non bit-field.
                return _enumValues.get(getByteBuffer().get(position()));
            } else { // Bit-field.
                int value = getByteBuffer().get(position());
                value &= _mask;
                value <<= _signShift;
                value >>= _signShift + _shift; // Keeps sign.
                return _enumValues.get(value);
            }
        }
     ...
     */
    public class Enum16<E extends Enum<?>> extends Member {
        private final int _mask;
        ...
        public Enum16(List<E> enumValues) {
            this(enumValues, 16);
        }

        public Enum16(List<E> enumValues, int nbrOfBits) {
            super(2, nbrOfBits, 16);
            _enumValues = enumValues;
            final int startBit = offset() << 3;
            _shift = (byteOrder() == ByteOrder.BIG_ENDIAN) ? 16 - _bitIndex
        ...
        public E get() {
            if (_mask == 0xFFFF) { // Non bit-field.
                return  _enumValues.get(getByteBuffer().getShort(
                        position()));
            } else { // Bit-field.
                int value = getByteBuffer().getShort(position());
                value &= _mask;
     ...
     */
    public class Enum32<E extends Enum<?>> extends Member {
        private final int _mask;
        ...
        public Enum32(List<E> enumValues) {
            this(enumValues, 32);
        }

        public Enum32(List<E> enumValues, int nbrOfBits) {
            super(4, nbrOfBits, 32);
            _enumValues = enumValues;
            final int startBit = offset() << 3;
            _shift = (byteOrder() == ByteOrder.BIG_ENDIAN) ? 32 - _bitIndex
        ...
        public E get() {
            if (_mask == 0xFFFFFFFF) { // Non bit-field.
                return  _enumValues.get(getByteBuffer()
                        .getInt(position()));
            } else { // Bit-field.
                int value = getByteBuffer().getInt(position());
                value &= _mask;
     ...
     */
    public class Enum64<E extends Enum<?>> extends Member {
        private final long _mask;
        ...
        public Enum64(List<E> enumValues) {
            this(enumValues, 64);
        }

        public Enum64(List<E> enumValues, int nbrOfBits) {
            super(8, nbrOfBits, 64);
            _enumValues = enumValues;
            final int startBit = offset() << 3;
            _shift = (byteOrder() == ByteOrder.BIG_ENDIAN) ? 64 - _bitIndex
        ...
        public E get() {
            if (_mask == 0xFFFFFFFFFFFFFFFFl) { // Non bit-field.
                return  _enumValues.get((int) getByteBuffer().getLong(
                        position()));
            } else { // Bit-field.
                long value = getByteBuffer().getLong(position());
                value &= _mask;
                value <<= _signShift;
                value >>= _signShift + _shift; // Keeps sign.
                return  _enumValues.get((int) value);
            }
        }
